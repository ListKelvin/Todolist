...
//////////////////////////////////////////////////////////
  ...
  return (
    ...
    <div className="App">
        <p>To Do lisst</p>
        <input className='input' value={work}  onChange={(event) => setwork(event.target.value)}/>
        <button className='btn' onClick={handleAddWork} >ADD</button>

        <ul >
          {listWorks.map((item,index)=>(
              <li key={index}> {index + 1} - {item}  <button className='btn' onClick={()=> handleDelete(item)}>DELETE</button> </li> 
          ))}  
        </ul>
    </div>
  );
